...
/*-------------------------------------------------------------------
  ...
  -------------------------------------------------------------------*/
GR_ENTRY(grSstWinOpen, GrContext_t, (FxU32 hWnd, GrScreenResolution_t resolution, GrScreenRefresh_t refresh, GrColorFormat_t format, GrOriginLocation_t origin, int nColBuffers, int nAuxBuffers))
{
...
GR_EXT_ENTRY(grSstWinOpenExt, GrContext_t, (FxU32 hWnd, GrScreenResolution_t resolution, GrScreenRefresh_t refresh, GrColorFormat_t format, GrOriginLocation_t origin, GrPixelFormat_t pixelformat, int nColBuffers, int nAuxBuffers))
{
  ...
  */
...

  {
    ...
    */
    ...
    /* nd Extended */
    GDBG_INFO(80, "nejc0 glideResOverride value: %d\n", _GlideRoot.environment.glideResOverride);
    ...
    {
      GDBG_INFO(80, "nejc0 override hit resolution: %d\n", _GlideRoot.environment.glideResOverride);
    ...
    if (driInfo.cpp == 3 || driInfo.cpp == 4)
    { /* 24 or 32bpp modes */
    ...
    */
    ...
    if (gc->sliCount > 1)
    {
       ...
       * out as zero. */
      /* nejc if(!_GlideRoot.environment.is_opengl )
     This will cause problems with mesaFx if sliBandHeightForce = true
      */
      if (!_GlideRoot.environment.sliBandHeightForce)
      {
    ...
    /* If we are doing 2 or 4 sample AA, then each chip needs twice as many buffers */
            ...
            MIN_TEXTURE_STORE + MIN_FIFO_SIZE) >
        (gc->bInfo->h3Mem << 20))
    {
    ...
    */
...
#ifdef DRI_BUILD
      REG_GROUP_SET(hw, colBufferStride, (!gc->curBuffer) ? (FxU32)driInfo.stride : gc->state.shadow.colBufferStride);
#else
...
#if 1
    {
      ...
      REG_GROUP_BEGIN(BROADCAST_ID, colBufferAddr, 4, 0xf);
      {
        REG_GROUP_SET(hw, colBufferAddr, gc->state.shadow.colBufferAddr);
#ifdef DRI_BUILD
        REG_GROUP_SET(hw, colBufferStride, (!gc->curBuffer) ? driInfo.stride : gc->state.shadow.colBufferStride);
#else
        REG_GROUP_SET(hw, colBufferStride, gc->state.shadow.colBufferStride);
#endif
        REG_GROUP_SET(hw, auxBufferAddr, gc->state.shadow.auxBufferAddr);
...
** chipid is 0 based
*/
void _grAAOffsetValue(FxU32 *xOffset,
                      FxU32 *yOffset,
                      FxU32 minchipid,
                      FxU32 maxchipid,
...
void _grRenderMode(FxU32 pixelformat)
{
